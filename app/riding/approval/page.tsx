...
export default function RideApprovalPage() {
  ...
  const load = async () => {
    ...
    const [{ data: accountData }, { data: rideData, error: rideError }, { data: memberData }] = await Promise.all([
      supabase.from("member_accounts").select("role").maybeSingle(),
  ...
  if (!canReview(account?.role)) return <AppShell active="Validasi Ride" title="Validasi Riding"><div className="page-wrap"><section className="empty-state card"><ShieldAlert/><h2>Akses Road Captain diperlukan</h2><p>Halaman validasi riding hanya tersedia untuk Road Captain, Admin, dan Superadmin.</p><a className="primary-action" href="/login">MASUK</a></section></div></AppShell>;
