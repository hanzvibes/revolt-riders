...
export default function CashPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [{ data: accountData }, { data: summaryData, error: summaryError }] = await Promise.all([
  ...
  return <AppShell active="Kas Revolt" title="Kas Revolt"><div className="page-wrap">
    ...
    {message ? <div className="card empty-state"><CircleDollarSign/><h2>{message}</h2>{message.startsWith("Masuk") && <a className="primary-action" href="/login">MASUK KE AKUN</a>}</div> : <>
